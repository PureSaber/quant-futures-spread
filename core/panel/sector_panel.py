"""core/panel/sector_panel.py —— SpreadSectorPanel：日收盘截面 + sector extreme cache。"""
...
from core.data.spread_schedule import calendar_by_products
from core.panel.calendar_panel import (
...
class SpreadSectorPanel:
    ...
    calendar_by_product: Dict[str, Dict[str, str]] = field(default_factory=dict)
    ...
    def board_peers_for(self, spread_symbol: str, tradingday=None) -> List[str]:
        sym = str(spread_symbol)
        cal = getattr(self, "calendar_by_product", None) or {}
        if cal and tradingday is not None:
            return calendar_board_peers(
                sym,
                tradingday,
                cal,
                self.minute_by_spread,
                self.industry_map,
                self.sector_map,
                self.spread_to_sector,
            )
        peers = self.board_peers.get(sym)
        return list(peers) if peers else ([sym] if sym else [])

    def day_universe(self, tradingday) -> List[str]:
        universe = list(self.minute_by_spread.keys())
        cal = getattr(self, "calendar_by_product", None) or {}
        if cal:
            active = active_spreads_on_day(cal, tradingday, universe)
            return active if active else universe
        return universe
    ...
    def morning_pool_masks(self, pool_mode: str = "cv80"):
        ...
        cal = getattr(self, "calendar_by_product", None) or None
        result = build_morning_pool_masks(
            self.minute_by_spread, symbols, pool_mode, calendar_by_product=cal,
        )
...
def build_panel_from_schedule(
    ...
    if not daily_frames and not minute_frames:
        ...
        cal_empty = calendar_by_products(schedule.calendar, products, years, schedule.tenor)
        return SpreadSectorPanel(
            sector_map=dict(sector_map),
            industry_map=industry_map,
            calendar_by_product=cal_empty,
        )

    cal_map = calendar_by_products(schedule.calendar, products, years, schedule.tenor)
    daily_close = build_product_daily_close(cal_map, daily_frames)
    if daily_close.empty and daily_frames:
        daily_close = pd.DataFrame(daily_frames).sort_index()
...
def strategy_needs_panel(module_path: str) -> bool:
    ...
    return any(
        name in mod
        for name in ("zhouhaotian", "xuhe", "tangye2", "gaozong2", "wangzhihao")
    )
