"""core/data/market_store.py — 内存 K 线库。"""
...
class InMemoryMarketStore:
    ...
    def clear_symbol(self, symbol: str, exchange: str = "", interval: str = "1m", source: str = "") -> None:
        """换 spread 时释放旧 symbol 的内存 bar。"""
        prefix = (symbol or "").strip().lower()
        if not prefix:
            return
        drop = [k for k in self._bars if k[0] == prefix]
        for k in drop:
            del self._bars[k]
        ...
        return [
            {
                ...
                "close_50": b.close_50,
                "close_80": b.close_80,
                "bid_price_20": b.bid_price_20,
                "bid_price_50": b.bid_price_50,
                "bid_price_80": b.bid_price_80,
                "ask_price_20": b.ask_price_20,
                "ask_price_50": b.ask_price_50,
                "ask_price_80": b.ask_price_80,
