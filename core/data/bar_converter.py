...
def df_to_bars(
    ...
    bid_20_s = _opt_col(out, "bidPrice_20", pd.Series(0.0, index=out.index))
    bid_50_s = _opt_col(out, "bidPrice_50", pd.Series(0.0, index=out.index))
    bid_80_s = _opt_col(out, "bidPrice_80", pd.Series(0.0, index=out.index))
    ask_20_s = _opt_col(out, "askPrice_20", pd.Series(0.0, index=out.index))
    ask_50_s = _opt_col(out, "askPrice_50", pd.Series(0.0, index=out.index))
    ask_80_s = _opt_col(out, "askPrice_80", pd.Series(0.0, index=out.index))
    bid_vol_s = _opt_col(out, "bidVolume", pd.Series(0.0, index=out.index))
    ask_vol_s = _opt_col(out, "askVolume", pd.Series(0.0, index=out.index))
    ...
    for idx in range(len(out)):
        ...
        bars.append(BarData(
            ...
            bid_price_20=float(bid_20_s.iloc[idx]),
            bid_price_50=float(bid_50_s.iloc[idx]),
            bid_price_80=float(bid_80_s.iloc[idx]),
            ask_price_20=float(ask_20_s.iloc[idx]),
            ask_price_50=float(ask_50_s.iloc[idx]),
            ask_price_80=float(ask_80_s.iloc[idx]),
            bid_volume=float(bid_vol_s.iloc[idx]),
            ask_volume=float(ask_vol_s.iloc[idx]),
